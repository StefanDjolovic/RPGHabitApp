...
type SandboxSummary = { items: number; gold: number; classes: number; activeClass: string | null };
...
export default function AdminLabScreen() {
  ...
  const [summary, setSummary] = useState<SandboxSummary>({ items: 0, gold: 0, classes: 0, activeClass: null });
  const [working, setWorking] = useState<'unlock' | 'refill' | 'class' | null>(null);
  ...
  const runRefill = async () => {
    ...
    try {
      ...
      await load();
      setMessage('Gold and every inventory item were refilled.');
    } catch (error) {
  ...
  return (
    <ScrollView
      ...
      <View style={styles.summaryBand}>
        <View style={styles.summaryStat}><Text style={styles.summaryValue}>{summary.classes}/5</Text><Text style={styles.summaryLabel}>CLASSES</Text></View>
        <View style={styles.divider} />
        <View style={styles.summaryStat}><Text style={styles.summaryValue}>{summary.items}</Text><Text style={styles.summaryLabel}>ITEM TYPES</Text></View>
        <View style={styles.divider} />
      ...
      <View style={styles.warningPanel}>
        ...
        <Text style={styles.warningText}>Sandbox grants are permanent for this admin account and are included in its cloud backup.</Text>
      </View>
